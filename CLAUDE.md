...
`WalletTransactionIntent.swift` implements `AppIntent` for Shortcuts automation. When triggered from Apple Wallet, it creates a `Transaction` by opening its own `ModelContainer` (not the shared one), parses the amount string via `CurrencyUtils.parseCurrencyAndAmount(from:)` to extract currency and amount, guesses category via `CategoryInference.infer(...)`, logs to Firebase Analytics via `AnalyticsTracker`, and fires a `UNUserNotificationCenter` notification.

### Category Inference
`CategoryInference` (`Utils/CategoryInference.swift`) runs from two places: `WalletTransactionIntent` for Wallet taps, and `TransactionFormView` when the merchant field loses focus while **adding** a transaction (never when editing, and never over a category the user picked by hand). It decides a new transaction's category in three tiers: (1) a past transaction from the same merchant, (2) the remote category API, (3) a local keyword heuristic that always yields at least `"Other"`. Tier 2 POSTs the merchant name plus **all** `SpendingCategory` records (custom ones included, each with an icon-derived hint) to the Next.js service in `backend/`, which asks a TypeSafe Jev `choice` question. The answer is only used when `confidence >= CategoryInference.minimumConfidence` and its name matches a stored category — the returned value is always the stored record's canonical `name`, because `MerchantUtils.normalizedCategory` collapses unrecognised strings to `"Other"`. The tier that answered is reported as `category_source` on the `add_wallet_transaction` analytics event. Callers snapshot their `SpendingCategory` records into `CategoryInference.CategoryOption` values via `options(from:)` before awaiting `remoteCategory`, so SwiftData models never cross the await. In the manual form the category fills in silently (a spinner shows while the remote tier is in flight) and a hand-picked category is never overwritten. This replaced an `NLEmbedding` similarity match; do not reintroduce Apple's NL APIs for categorisation.

`AppConfig` (`Utils/AppConfig.swift`) supplies the API host and bearer token from `Info.plist`, which Xcode fills from the gitignored `Config/Secrets.xcconfig` (see `Config/Secrets.example.xcconfig`). This repo is open source: **never hardcode the endpoint host, the bearer token, or the TypeSafe API key**. The TypeSafe key is server-side only, set as a Vercel environment variable. When config is absent, `AppConfig` returns nil and tier 2 is skipped, so the app works unmodified from a fresh clone.

### Backend (`backend/`)
A minimal Next.js App Router project deployed to Vercel, with one route: `POST /api/categorize`. It authenticates a shared bearer token (`CATEGORIZE_API_TOKEN`), validates the body, and calls TypeSafe Jev with `TYPESAFE_API_KEY`. Both are required environment variables; missing either returns `500 server_misconfigured`. See `backend/README.md`. Run `npm run build` in `backend/` to typecheck.
