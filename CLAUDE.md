...
### Category Inference
`CategoryInference` (`Utils/CategoryInference.swift`) decides a new transaction's category in three tiers: (1) a past transaction from the same merchant, (2) the remote category API, (3) a local keyword heuristic that always yields at least `"Other"`. Tier 2 POSTs the merchant name plus **all** `SpendingCategory` records (custom ones included, each with an icon-derived hint) to the Next.js service in `backend/`, which asks a TypeSafe Jev `choice` question. The answer is only used when `confidence >= CategoryInference.minimumConfidence` and its name matches a stored category — the returned value is always the stored record's canonical `name`, because `MerchantUtils.normalizedCategory` collapses unrecognised strings to `"Other"`. The tier that answered is reported as `category_source` on the `add_wallet_transaction` analytics event. This replaced an `NLEmbedding` similarity match; do not reintroduce Apple's NL APIs for categorisation.
