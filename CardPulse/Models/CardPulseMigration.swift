...
//
// V7 freezes `Card`, `Transaction`, and `CardRewardRule` so V8's addition of
// `maxRewardCap` to the live `CardRewardRule` doesn't change V7's per-stage checksum.
...
// MARK: - V8 Schema (current — CardRewardRule gains `maxRewardCap`)
...
enum CardPulseMigrationPlan: SchemaMigrationPlan {
    ...
    /// V7 → V8: add `maxRewardCap` to `CardRewardRule`. Defaults to `0` (no per-category
    /// cap), so SwiftData handles this as a lightweight migration and existing cards are
    /// unaffected.
